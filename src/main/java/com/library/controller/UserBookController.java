...
public class UserBookController {
	...
	public ResponseEntity<List<AvailableBooks>> getAvailableBooksByName(@PathVariable(value = "name") String name){	
	...
	public ResponseEntity<List<AvailableBooks>> getAvailableBooksByAuthor(@PathVariable(value = "name") String name){
		return ResponseEntity.ok().body(bookService.getAvailableBooksByAuthor(name));
	}
	
	// add books to user
	@PutMapping("/books/add/{id}")
	public ResponseEntity<Book> addUserBook(@PathVariable(value = "id") long bookId){
